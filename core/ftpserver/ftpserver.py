...
class FtpServer(socketserver.StreamRequestHandler):
    ...
    def handle(self):
        """处理客户端请求"""
        while True:
            head_dict = json.loads(self.connection.recv(8192).decode('utf-8'))
            action = head_dict.get("action", 0)
            if not action:
                self.connection.send(b'9999')
            else:
                if hasattr(self, action):
                    func = getattr(self, action)
                    func(head_dict)
                else:
                    self.connection.send(b'9998')
    ...
    def put(self, cmd_dict):
        ...
        with open(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'home', filename), 'wb') as f:
            recv_size = 0
            while recv_size < size:
                data = self.connection.recv(min(size - recv_size, 8192))
                f.write(data)
